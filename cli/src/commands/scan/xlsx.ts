...
export default class ScanXlsx extends Command {
  static args = {
    file: Args.string({
      description: 'Excel spreadsheet (.xlsx) file or directory to scan',
      required: true,
    }),
  }
static description = 'Scan Excel spreadsheets (.xlsx) for accessibility issues'
static examples = [
    '<%= config.bin %> <%= command.id %> spreadsheet.xlsx',
    '<%= config.bin %> <%= command.id %> ./spreadsheets/',
    '<%= config.bin %> <%= command.id %> data.xlsx --format json',
    '<%= config.bin %> <%= command.id %> ./excel/ --api-url http://localhost:8000',
  ]
static flags = {
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(ScanXlsx)
    const startTime = Date.now()

    intro('Aelira CLI - Excel Spreadsheet Accessibility Scanner')
  ...
  private displayBatchResults(results: any[], totalTime: number): void {
    this.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    this.log(`  Batch Excel Scan Results`)
    this.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n')
  ...
  private displaySingleResult(result: any, scanTime: number): void {
    this.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    this.log(`  Excel Spreadsheet Accessibility Report`)
    this.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n')

    this.log(`  File: ${result.filename || 'Unknown'}`)
    this.log(`  Sheets: ${result.sheet_count || 'N/A'}`)
    this.log(`  Total Cells: ${result.total_cells || 'N/A'}`)
    this.log(`  Charts: ${result.chart_count || 0}`)
  ...
  private async findXlsxFiles(dirPath: string): Promise<string[]> {
    const files: string[] = []
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        files.push(...(await this.findXlsxFiles(fullPath)))
  ...
  private async scanDirectory(
    ...
    const s = spinner()
    s.start('Finding Excel spreadsheets...')

    const files = await this.findXlsxFiles(dirPath)

    if (files.length === 0) {
      s.stop('No Excel spreadsheets found')
      outro('⚠️  No .xlsx files to scan')
      return
    }

    s.stop(`Found ${files.length} spreadsheet${files.length > 1 ? 's' : ''}`)
    ...
    if (flags.format === 'csv') {
      ...
      outro(`✨ Batch scan complete! Scanned ${files.length} spreadsheets`)
    }
  ...
  private async scanFile(
    ...
    const s = spinner()
    s.start('Uploading Excel spreadsheet to Aelira API...')
    ...
    try {
      ...
      if (flags.format === 'csv') {
        ...
        if (flags.output) {
          await fs.writeFile(flags.output, JSON.stringify(output, null, 2))
          outro(`✅ Excel scan complete. Report saved to ${flags.output}`)
        } else {
          this.log(JSON.stringify(output, null, 2))
        }
      } else {
        this.displaySingleResult(result, scanDuration)
        outro('✨ Excel spreadsheet scan complete!')
      }
  ...
  private async uploadAndScan(
    ...
    const response = await api.postForm('/education/excel/scan', formData as any, {
      timeout: 60_000,
    })
