...
def upgrade() -> None:
    ...
    op.add_column(
        'waitlist_signups',
        sa.Column('institution_domain', sa.String(255), nullable=True, comment='Domain extracted from email address')
    )
    op.add_column(
        'waitlist_signups',
        sa.Column('status', sa.String(50), server_default='new', nullable=False, comment='Lead status: new, contacted, demo_scheduled, pilot, converted, churned')
    )
    op.add_column(
        'waitlist_signups',
        sa.Column('notes', sa.Text, nullable=True, comment='Internal notes for sales/support team')
    )
    op.add_column(
        'waitlist_signups',
        sa.Column('contacted_at', sa.DateTime(timezone=True), nullable=True, comment='Timestamp of first outreach')
    )
...
def downgrade() -> None:
    ...
    # Drop indexes first
    op.drop_index('ix_waitlist_signups_institution_domain', table_name='waitlist_signups')
