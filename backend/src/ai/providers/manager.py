...
# Thread pool for running async code synchronously - configurable size
settings = get_settings()
_sync_executor = concurrent.futures.ThreadPoolExecutor(
    max_workers=getattr(settings, "llm_thread_pool_size", 4)
)
...
def _run_async_in_thread(coro):
    """Run an async coroutine synchronously using a thread pool.

    This is needed because asyncio.run() cannot be called from within
    an existing event loop (like when FastAPI is running).
    """
    ...
    future = _sync_executor.submit(run_in_new_loop)
...
class ProviderManager:
    ...
    def __init__(
        ...
        if fallback_provider is not None:
            ...
            self.fallback_type = self._get_env_provider(
    ...
    def _build_default_configs(self):
        ...
        if ProviderType.GEMINI not in self.configs:
            self.configs[ProviderType.GEMINI] = ProviderConfig(
                ...
                code_model=os.getenv("GEMINI_CODE_MODEL", "gemini-2.0-flash"),
        ...
        if ProviderType.OLLAMA not in self.configs:
            self.configs[ProviderType.OLLAMA] = ProviderConfig(
                ...
                code_model=os.getenv("OLLAMA_CODE_MODEL", "qwen2.5-coder:3b"),
