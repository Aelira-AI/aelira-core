...
- gemini-2.0-flash-exp: 87% accuracy, 1.8s avg - FASTEST (free tier default)
    ...
    def default_for_provider(cls, provider_type: ProviderType) -> "ProviderConfig":
        ...
        defaults = {
            ProviderType.GEMINI: cls(
                ...
                # Default: fastest model (free tier)
                # Paid users can upgrade to gemini-3-flash-preview (100% accuracy)
                # via PUT /llm/providers/gemini/models
                text_model="gemini-2.0-flash-exp",
                code_model="gemini-2.0-flash-exp",
                vision_model="gemini-2.0-flash-exp",
            ),
            ProviderType.OLLAMA: cls(
                provider_type=ProviderType.OLLAMA,
                host="http://localhost:11434",
                text_model="qwen2.5-coder:1.5b",  # Fast classification (60% accuracy)
                code_model="qwen2.5-coder:3b",  # Best code gen (100% accuracy)
                vision_model="minicpm-v:latest",  # Best vision (54% vs moondream 10%)
...
PROVIDER_MODELS: Dict[ProviderType, Dict[str, ModelConfig]] = {
    ProviderType.GEMINI: {
        ...
        "gemini-2.5-flash": ModelConfig(
            ...
            supports_vision=True,
            context_window=1048576,
            description="87% accuracy, 5.4s avg - balanced option",
        ),
        # Fastest option (free tier)
        "gemini-2.0-flash-exp": ModelConfig(
            name="gemini-2.0-flash-exp",
            max_tokens=8192,
            supports_vision=True,
            context_window=1048576,
            description="87% accuracy, 1.8s avg - FASTEST (free tier)",
        ),
