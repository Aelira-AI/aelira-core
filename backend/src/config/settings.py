...
class Settings(BaseSettings):
    ...
    def validate_cookie_domain(cls, v, values):
        ...
        if env == "production" and not v:
            ...
            warnings.warn(
                ...
                "Set SESSION_COOKIE_DOMAIN=.aelira.ai for cross-subdomain auth."
            )
        return v
    ...
    # =====================================================
    # Gemini Model Configuration (Jan 2026 Benchmarks)
    # =====================================================
    # Available models (accuracy / speed):
    #   - gemini-3-flash-preview: 100% accuracy, 5.4s - BEST QUALITY
    #   - gemini-2.5-flash: 87% accuracy, 5.4s
    #   - gemini-2.0-flash-exp: 87% accuracy, 1.8s - FASTEST
    #
    # Default: gemini-2.0-flash-exp (fastest for free tier)
    # Paid users can upgrade via PUT /llm/providers/gemini/models
    ...
    gemini_code_model: str = os.getenv("GEMINI_CODE_MODEL", "gemini-2.0-flash-exp")

    # Fallback to Ollama if Gemini unavailable
    use_gemini: bool = os.getenv("USE_GEMINI", "true").lower() == "true"
    # Updated Jan 2026: minicpm-v (54% accuracy) replaces llava (49%)
    ollama_fallback_vision: str = os.getenv(
        "OLLAMA_FALLBACK_VISION", "minicpm-v:latest"
    )
    ollama_fallback_text: str = os.getenv("OLLAMA_FALLBACK_TEXT", "qwen2.5-coder:1.5b")
    ...
    magic_link_base_url: str = os.getenv(
        "MAGIC_LINK_BASE_URL", "https://dashboard.aelira.ai"
    )
    ...
    google_oauth_redirect_uri: str = os.getenv(
        "GOOGLE_OAUTH_REDIRECT_URI", "https://api.aelira.ai/auth/google/callback"
    )
    ...
    microsoft_oauth_redirect_uri: str = os.getenv(
        "MICROSOFT_OAUTH_REDIRECT_URI", "https://api.aelira.ai/auth/microsoft/callback"
    )
    ...
    stripe_success_url: str = os.getenv(
        "STRIPE_SUCCESS_URL", "https://dashboard.aelira.ai/settings?upgrade=success"
    )
    stripe_cancel_url: str = os.getenv(
        "STRIPE_CANCEL_URL", "https://dashboard.aelira.ai/settings?upgrade=cancelled"
    )
    ...
    # Zammad Helpdesk (help.aelira.ai)
