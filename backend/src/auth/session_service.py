...
class SessionService:
    ...
    def get_or_create_user_for_magic_link(
        ...
        # Create new user with individual_free tier department
        # First, find or create a default department for individual users
    ...
    def _sync_signup_to_crm(
        ...
        """Sync new magic link signup to CRM (fire and forget)."""
        try:
            from src.integrations.crm_sync import get_crm_sync_service

            crm_service = get_crm_sync_service()
            if not crm_service or not crm_service.is_enabled:
                return
            ...
            loop.create_task(
                crm_service.sync_individual_signup(
                    email=email,
                    name=name,
                    institution=institution,
                    tier="individual_free",
            ...
            logger.info(f"CRM sync queued for new signup: {email}")
        except Exception as e:
            logger.warning(f"Failed to sync signup to CRM for {email}: {e}")
    ...
    def _get_or_create_individual_department(
        ...
        Individual users get their own "department" with individual_free tier.
