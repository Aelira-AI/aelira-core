...
from src.api.lti_routes import router as lti_router  # Phase 4.5: Canvas LTI
...
if sentry_dsn:
    ...
    sentry_sdk.init(
        ...
        traces_sampler=lambda ctx: (
            0 if ctx.get("asgi_scope", {}).get("path") == "/health" else None
        ),
    )
    logger.info(
...
app.include_router(lti_router)  # Phase 4.5: Canvas LTI
...
async def metrics(
    request: Request,
    admin_info: Tuple = Depends(get_admin_api_key),
):
    """
    Prometheus metrics endpoint for monitoring.

    Requires admin authentication to prevent information disclosure.
    Scraped by Prometheus for observability and alerting.
    Not included in OpenAPI schema (internal use only).

    Returns metrics in Prometheus text format including:
    - HTTP request counts and latencies
    - Document scan metrics
    - AI/LLM request metrics
    - Business metrics (quota usage, active users)
    """
    return Response(
...
# Served at api.aelira.ai/static/logo.png for use in email templates
...
if dashboard_path.exists():
    ...
    # Mount static assets at /assets for dashboard.aelira.ai subdomain (root-level)
    app.mount(
        "/assets",
        StaticFiles(directory=str(dashboard_path / "assets")),
        name="root-assets",
    ...
    # Root-level SPA routes for dashboard.aelira.ai subdomain
