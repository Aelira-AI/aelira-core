...
def _render_lti_error_page(
    ...
    if show_upgrade_button:
        pricing_url = os.getenv("WEBSITE_URL", "https://aelira.ai") + "/pricing"
        upgrade_button = f"""
...
async def lti_launch(
    ...
    try:
        ...
        session_token = _create_lti_session(launch_data, department.id)
...
async def lti_health(lti_service: CanvasLTIService = Depends(get_lti_service)):
    ...
    return {
        ...
        "message": (
            "LTI integration ready"
            if lti_service.is_configured()
            else "Set CANVAS_CLIENT_ID and CANVAS_DEPLOYMENT_ID to enable"
        ),
    }
