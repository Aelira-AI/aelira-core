...
async def get_scan_details(
    ...
    if scan.result and scan.result.issues:
        logger.info(
            f"[GET SCAN] Scan {scan_id}: issues type={type(scan.result.issues)}, has_details={'details' in scan.result.issues if isinstance(scan.result.issues, dict) else 'N/A'}"
        )
        if isinstance(scan.result.issues, dict) and "details" in scan.result.issues:
            logger.info(
                f"[GET SCAN] Scan {scan_id}: details length={len(scan.result.issues['details'])}"
            )

    issues_details = []
    if scan.result:
        if isinstance(scan.result.issues, dict):
            issues_details = scan.result.issues.get("details", [])
            logger.info(
                f"[GET SCAN] Scan {scan_id}: Extracted {len(issues_details)} issues from dict"
            )
        elif isinstance(scan.result.issues, list):
            issues_details = scan.result.issues
            logger.info(
                f"[GET SCAN] Scan {scan_id}: Using {len(issues_details)} issues from list"
            )
        else:
            logger.warning(
                f"[GET SCAN] Scan {scan_id}: Unexpected issues type: {type(scan.result.issues)}"
            )
