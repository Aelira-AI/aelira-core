...
async def connect_canvas(
    ...
    # Store state in session/database for verification (simplified here)
    # In production, store state in Redis with TTL
    ...
    logger.info(
        f"Initiated Canvas OAuth for department {request.department_id} at {request.canvas_instance_url}"
    )
...
async def canvas_oauth_callback(
    ...
    canvas_instance_url: str = Query(
        ..., description="Canvas instance URL (passed via state)"
    ),
    ...
    try:
        ...
        api_client = CanvasAPIClient(
            canvas_instance_url=credential.canvas_instance_url,
...
async def canvas_connection_status(
    ...
    return CanvasConnectionStatus(
        ...
        user_email=credential.metadata.get("user_email"),
...
async def remediate_canvas_file(
    ...
    try:
        ...
        cloud_file = CloudFile(
            ...
            file_name=f"canvas_file_{request.file_id}",  # Will be updated after download
            mime_type="unknown",
            size_bytes=0,
        ...
        remediation_job = CloudJobQueue(
            ...
            job_data={
                "cloud_file_id": cloud_file.id,
                "course_id": request.course_id,
                "upload_to_cloud": request.upload_back,
                "use_ai": request.use_ai,
            },
            cloud_file_id=cloud_file.id,
