...
class AccountDeletionService:
    ...
    def deactivate_account(
        ...
        # Block if paid subscription is active
        if department and department.subscription_status == "active":
            raise ValueError(
                "Please cancel your subscription through the billing portal before deactivating your account."
            )
        ...
        if department and department.tier in (
            "individual_free",
            "individual_plus",
            "individual_pro",
        ):
    ...
    def confirm_deletion(
        ...
        if (
            department
            and department.stripe_subscription_id
            and department.subscription_status == "active"
        ):
            try:
                import stripe

                stripe.Subscription.cancel(department.stripe_subscription_id)
                department.subscription_status = "cancelled"
                logger.info(
                    f"Cancelled Stripe subscription {department.stripe_subscription_id} for deletion"
                )
            except Exception as e:
    ...
    def execute_scheduled_deletion(self, db: DBSession, user_id: str) -> bool:
        ...
        user.email = f"deleted-{user.id}@deleted.aelira.ai"
