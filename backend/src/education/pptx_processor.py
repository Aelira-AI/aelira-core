...
class AnimationIssue(BaseModel):
    """Animation accessibility issue (Task 8 - WCAG 2.2.2, 2.3.1)"""
...
class EmbeddedMedia(BaseModel):
    """Embedded media in PPTX (Task 9 - WCAG 1.2.1, 1.2.2, 1.2.3)"""
...
class EmbeddedMediaIssue(BaseModel):
    """Embedded media accessibility issue (Task 9)"""
...
class SlideAccessibilityIssues(BaseModel):
    ...
    animation_issues: List[AnimationIssue] = []  # Animation issues (Task 8)
    embedded_media_issues: List[EmbeddedMediaIssue] = (
        []
    )  # Embedded media issues (Task 9)
    total_issues: int
                ...
                all_issues.append(
                    {
                        ...
                        "title": (
                            ...
                            else "Inaccurate Alt Text"
                        ),
                        "description": f"Image '{issue.shape_name}' {'is missing' if not issue.has_alt_text else 'has inaccurate'} alt text",
                        "location": f"{slide_loc}, Shape '{issue.shape_name}'",
                        "wcag_criterion": "WCAG 1.1.1",
                        "suggested_fix": issue.suggested_alt_text
                        or "Add descriptive alt text",
...
class PowerPointProcessor:
    ...
    def process_pptx(self, file_path: str) -> PowerPointProcessingResult:
        ...
        for slide_idx, slide in enumerate(prs.slides, start=1):
            ...
            # Analyze slide animations (Task 8 - WCAG 2.2.2, 2.3.1)
            animation_issues = self._analyze_animations(slide, slide_idx, file_path)

            # Check embedded media (Task 9 - WCAG 1.2.1, 1.2.2, 1.2.3)
