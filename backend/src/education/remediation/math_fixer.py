...
class MathFixer:
        ...
        if not HAS_LATEX2MATHML:
            ...
            return [
                MathFixResult(
                    ...
                    equation_text=getattr(issue.metadata, "get", lambda k, d=None: d)(
                        "equation_text", ""
                    )
    ...
    def _fix_math_issue(self, issue: Any) -> MathFixResult:
        ...
        if not mathml_string:
            ...
            escaped = equation_text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            mathml_string = (
    ...
    def _extract_equation_from_page(self, page_num: int) -> str:
        ...
        try:
            ...
            logger.debug(f"MathFixer: could not extract text from page {page_num}: {exc}")
            return ""
