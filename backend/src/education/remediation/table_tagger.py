...
class TableTagger:
    ...
    def tag_table(
        ...
        if not HAS_PYMUPDF or not HAS_PIKEPDF:
            return TableTagResult(
                success=False, error="PyMuPDF and pikepdf required"
            )
    ...
    def _detect_tables(self, doc) -> List[TableInfo]:
        ...
        for page_num in range(len(doc)):
            ...
            try:
                ...
                logger.warning(
                    "Table detection failed on page %d: %s", page_num, exc
                )
        return tables
    ...
    def _confirm_headers_with_ai(
        self, pdf_path: str, tables: List[TableInfo]
    ) -> None:
        ...
        try:
            for table in tables:
                ...
                try:
                    ...
                    logger.debug(
                        "AI header confirmation failed for table: %s", exc
                    )
        finally:
            doc.close()
                    ...
                    if isinstance(row_idx, int) and 0 <= row_idx < table.rows:
                        for c in table.cells:
                            if c.row == row_idx:
                                c.is_header = True
                                c.scope = "Column"
                        table.header_rows = max(
                            table.header_rows, row_idx + 1
                        )
                    ...
                    if isinstance(col_idx, int) and 0 <= col_idx < table.cols:
                        for c in table.cells:
                            if c.col == col_idx:
                                c.is_header = True
                                c.scope = "Row"
                        table.header_cols = max(
                            table.header_cols, col_idx + 1
                        )
    ...
    def _build_table_element(self, pdf, table: TableInfo):
        ...
        page_obj = (
            pdf.pages[table.page_num].obj
            if table.page_num < len(pdf.pages)
            else None
        )
    ...
    def _build_row(
        ...
        for cell in row_cells:
            cell_tag = (
                Name.TH if (cell.is_header or is_header_row) else Name.TD
            )
