...
"""
    ...
    def __init__(self):
        ...
        if self.lualatex_available:
            ...
            logger.info("LaTeXML available - using MathML pipeline (PDF less accessible)")
        else:
            logger.warning("LuaLaTeX/LaTeXML not available - falling back to pandoc/pdflatex")
                    ...
                    if char == "{":
                        ...
                        if brace_count == 0:
                            ...
                            while end_pos < len(content) and content[end_pos] in "\n\r":
                                end_pos += 1
                            content = content[:start] + "% DocumentMetadata removed for LaTeXML processing\n" + content[end_pos:]
                            break
                else:
    ...
    async def _html_to_pdf_playwright(
    ...
    def _html_to_pdf_playwright_sync(
        self, html_path: str, pdf_path: str
    ) -> bool:
    ...
    def _convert_with_lualatex(
        ...
        try:
            # Run lualatex with shell-escape for tagpdf features
            ...
            for run in range(2):
                result = subprocess.run(
                    [
                        "lualatex",
                        "-interaction=nonstopmode",
                        "-shell-escape",  # Required for some tagpdf features
                ...
                if result.returncode != 0:
                    ...
                    logger.warning(f"LuaLaTeX pass {run+1} warnings: {result.stderr[:500]}")
        ...
        try:
            ...
            with pikepdf.open(pdf_path) as pdf:
                ...
                def has_content_ref(elem, depth=0) -> bool:
                    ...
                    if "/K" in elem:
                        kids = elem["/K"]
                        if isinstance(kids, (int, pikepdf.Object)) and not isinstance(kids, pikepdf.Dictionary):
                ...
                if isinstance(root_kid, pikepdf.Array):
                    for kid in root_kid:
                        if isinstance(kid, pikepdf.Dictionary) and has_content_ref(kid):
                            logger.info("PDF/UA structure verified - valid content references found")
                            return True
