...
class DocxRemediator(BaseRemediator):
    ...
    def _set_image_alt_text(self, run, alt_text: str):
        """Set alt text on an image in a run."""
        # Find the drawing element (parent of a:blip)
        # python-docx registers namespaces internally, so use qn() not namespaces=
        drawings = run._element.xpath(".//a:blip/..")

        for drawing in drawings:
            # Find or create docPr element
            doc_pr = drawing.xpath(f".//{qn('wp:docPr')}")
            if doc_pr:
                doc_pr[0].set("descr", alt_text)

    def _set_inline_shape_alt_text(self, shape_element, alt_text: str):
        """Set alt text on an inline shape element."""
        # Find docPr element using qn() for namespace-qualified name
        doc_pr = shape_element.xpath(f".//{qn('wp:docPr')}")
        if doc_pr:
            doc_pr[0].set("descr", alt_text)
        else:
    ...
    def _apply_heading_fix(
        ...
        try:
            para_index = issue.metadata.get("paragraph_index")
            suggested_level = issue.metadata.get("suggested_level", 2)
            ...
            # Get the heading style name
            style_name = self.HEADING_STYLES.get(suggested_level, "Heading 2")

            # Apply the style
            try:
                paragraph.style = style_name
            except KeyError:
                # Style doesn't exist, create it or use built-in
                paragraph.style = document.styles["Heading 2"]

            logger.info(f"Applied {style_name} to paragraph {para_index}")
            return True
    ...
    def _apply_heading_by_text(self, document: Document, text: str, level: int) -> bool:
        ...
        for paragraph in document.paragraphs:
            if text.strip() in paragraph.text.strip():
                style_name = self.HEADING_STYLES.get(level, "Heading 2")
                try:
                    paragraph.style = style_name
                    logger.info(f"Applied {style_name} to paragraph: {text[:50]}...")
                    return True
                except KeyError:
                    paragraph.style = document.styles["Heading 2"]
                    return True
    ...
    def _apply_list_formatting(self, paragraph):
