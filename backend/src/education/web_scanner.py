...
2. Run WCAG 2.2 compliance checks using axe-core
...
# Human-friendly fix descriptions for ALL axe-core WCAG 2.2 Level AA rules (~90+ rules)
...
class WebScanner:
    """Scan websites for WCAG 2.2 accessibility compliance"""
    ...
    def _scan_page(self, context, url: str) -> WebPageScanResult:
        ...
        try:
            ...
            title = page.title()
            logger.info(
                f"[DEBUG] Page loaded: {url}, title: {title}, spa: {spa_framework.value}"
            )

            # Run axe-core accessibility scan with comprehensive options
            axe = Axe()
            logger.info(f"[DEBUG] Running axe-core scan on {url}")
            ...
            logger.info(f"[DEBUG] Parsed {len(issues)} issues from axe results")
            ...
            if self.scan_focus_order and self.focus_order_analyzer:
                logger.info(f"[DEBUG] Running focus order analysis for {url}")
                try:
            ...
            if self.scan_cvd and self.cvd_simulator:
                logger.info(f"[DEBUG] Running CVD analysis for {url}")
                try:
            ...
            try:
                ...
                if shadow_dom_detected:
                    logger.info(
                        f"[DEBUG] Scanning {shadow_dom_host_count} Shadow DOM hosts for {url}"
                    )
    ...
    def _parse_axe_results(
        ...
        logger.info(f"[DEBUG] _parse_axe_results - input type: {type(results)}")
        ...
        if hasattr(results, "response"):
            logger.info("[DEBUG] Unwrapping results.response")
        ...
        # If results is an AxeResults object, try accessing its __dict__
            ...
            logger.info(
                f"[DEBUG] Accessing AxeResults.__dict__: {results.__dict__.keys()}"
            )
            # Try to find the violations data in the object's dict
            if "_AxeResults__violation_report" in results.__dict__:
                logger.info("[DEBUG] Found __violation_report, extracting...")
        ...
        if isinstance(results, dict):
            logger.info(f"[DEBUG] Results is dict, keys: {list(results.keys())}")
            violations = results.get("violations", [])
            logger.info(f"[DEBUG] Got violations from dict, count: {len(violations)}")
            ...
            logger.info(
                f"[DEBUG] Passes: {passes_count}, Incomplete: {incomplete_count}, Inapplicable: {inapplicable_count}"
            )
        ...
        # Case 2: Object with violations attribute
            ...
            logger.info(
                f"[DEBUG] Got violations from attribute, count: {len(violations)}"
            )
        ...
        logger.info(f"[DEBUG] Total violations to process: {len(violations)}")
        if len(violations) > 0:
            logger.info(
                f"[DEBUG] First violation ID: {violations[0].get('id', 'unknown')}"
            )
        ...
        for idx, violation in enumerate(violations):
            ...
            logger.info(
                f"[DEBUG] Processing violation {idx+1}/{total_violations}: {violation_id}"
            )
            for node_idx, node in enumerate(violation.get("nodes", [])):
                logger.info(
                    f"[DEBUG] Processing node {node_idx+1} for violation {violation_id}"
                )
                element_html = node.get("html", "")[:200]  # Truncate long HTML
                logger.info(f"[DEBUG] Got element HTML for {violation_id}")
                failure_summary = node.get("failureSummary", "")
                logger.info(f"[DEBUG] Got failure summary for {violation_id}")

                # Extract location information from axe-core node
                # axe-core provides 'target' as an array of CSS selectors
                target = node.get("target", [])
                selector = (
                    ", ".join(target) if isinstance(target, list) and target else None
                )
                    ...
                    try:
                        ...
                        if first_selector:
                            ...
                            if element:
                                ...
                                logger.info(
                                    f"[DEBUG] Captured screenshot for element: {first_selector[:50]}"
                                )
                    except Exception as e:
                        logger.warning(
                ...
                logger.info(
                    f"[DEBUG] Creating contextual description for {violation_id}"
                )
                ...
                logger.info(
                    f"[DEBUG] Created contextual description for {violation_id}"
                )

                # Transform axe-core technical message into human-friendly guidance
                # Using dictionary mapping for fast, deterministic results
                # TODO: Add optional LLM enhancement in future with proper timeout/async handling
                logger.info(f"[DEBUG] Humanizing fix description for {violation_id}")
                enhanced_fix = (
                    humanize_fix_description(failure_summary)
                    if failure_summary
                    else failure_summary
                )
                logger.info(f"[DEBUG] Humanized fix description for {violation_id}")

                # Generate AI explanation AND code fix if enabled (single model call for both!)
                ai_explanation = None
                ai_code_fix = None
                violation_impact = violation.get("impact", "unknown")
                violation_id = violation.get("id", "unknown")
                logger.info(
                    f"[CODE FIX] Checking if should generate fix: use_ai_analysis={self.use_ai_analysis}, impact={violation_impact}, id={violation_id}"
                )

                if self.use_ai_analysis and violation.get("impact") in [
                    "critical",
                    "serious",
                    "moderate",
                    "minor",
                    ...
                    logger.info(
                        f"[CODE FIX] Generating AI explanation and code for {violation_id} (impact: {violation_impact})"
                    )
                    ai_explanation, ai_code_fix = self._generate_code_fix(
                        violation.get("description", ""),
                        element_html,
                        failure_summary,
                        violation.get("id", ""),
                        violation_impact,  # Pass impact level for smart model selection
                        page_context=page_context,  # Pass full page context for context-aware fixes
                    )
                    if ai_code_fix:
                        logger.info(
                            f"[CODE FIX] Successfully generated explanation and code for {violation_id}"
                        )
                        logger.info(
                            f"[CODE FIX] Explanation: {ai_explanation[:100] if ai_explanation else 'None'}..."
                        )
                        logger.info(f"[CODE FIX] Code: {ai_code_fix[:100]}...")
                    else:
                        logger.warning(
                            f"[CODE FIX] Failed to generate fix for {violation_id}"
                        )

                # Create issue with priority assignment
                # Use AI-generated explanation if available, otherwise use enhanced_fix
                final_explanation = ai_explanation if ai_explanation else enhanced_fix
                ...
                issue = WebPageIssue(
                    ...
                    fix=final_explanation,  # AI-generated explanation OR fallback
                    generated_code_fix=ai_code_fix,  # AI-generated code fix
                    page_url=page_url,
                    selector=selector,
                    xpath=xpath,
...
Now provide your response (remember: CODE section = ONLY HTML, NO TEXT):"""
            ...
            system_prompt = "You are an expert web accessibility developer specializing in WCAG 2.2 AA compliance. Generate production-ready HTML fixes with clear explanations."
            ...
            if not result.get("success"):
                ...
                return (
                    f"Fix this {issue_id} issue by following WCAG 2.2 AA guidelines.",
