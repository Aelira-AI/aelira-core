...
class PDFProcessor:
    ...
    def _ocr_pdf_enhanced(self, file_path: str) -> tuple:
        ...
        try:
            ...
            # PDF already has OCR text layer, just extract it
            logger.info(
                "[PDFProcessor] PDF already has OCR text layer, extracting directly"
            )
            text = self._extract_text(file_path)
        ...
        for elem_type, elem in all_elements:
            ...
            if elem_type == "heading":
                ...
                if text and text[0].isdigit() and text[1] in (".", ")"):
                    text = text[2:].lstrip()
                elif text and text[0].isalpha() and text[1] in (".", ")"):
                    text = text[2:].lstrip()
