...
def accessible_pdf():
    ...
    # Mark as tagged
    pdf.Root[pikepdf.Name.MarkInfo] = pikepdf.Dictionary(
        {"/Marked": True}
    )
...
class TestMatterhornValidatorChecks:
    ...
    def test_check_language_set(self, bare_pdf_path):
        ...
        lang_check = next(
            (cp for cp in result.checkpoints if cp.id == "06-001"), None
        )
    ...
    def test_check_marked_content(self, bare_pdf_path):
        ...
        mark_check = next(
            (cp for cp in result.checkpoints if cp.id == "01-004"), None
        )
    ...
    def test_check_title_missing(self, bare_pdf_path):
        ...
        title_check = next(
            (cp for cp in result.checkpoints if cp.id == "07-001"), None
        )
    ...
    def test_pdfua_identifier_missing(self, bare_pdf_path):
        ...
        ua_check = next(
            (cp for cp in result.checkpoints if cp.id == "06-003"), None
        )
