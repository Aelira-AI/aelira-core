...
from src.education.remediation.latex_converter import LaTeXConverter, get_latex_converter
...
class TestLuaLaTeXAvailability:
    ...
    def test_converter_detects_lualatex(self, latex_converter):
        ...
        # At least one conversion method should be available
        assert (
            latex_converter.lualatex_available or
            latex_converter.latexml_available or
            latex_converter.pdflatex_available
...
class TestDocumentMetadataInjection:
    ...
    def test_auto_remediate_adds_document_metadata(self, sample_latex_without_accessibility):
        ...
        assert doc_metadata_pos < documentclass_pos, "DocumentMetadata must come before documentclass"
    ...
    def test_removes_obsolete_packages(self, sample_latex_without_accessibility):
        ...
        assert r'\usepackage{accessibility}' not in remediated, "accessibility package should be removed"
        assert r'\usepackage{axessibility}' not in remediated, "axessibility package should be removed"

        print("\n✓ Obsolete packages correctly removed")


@pytest.mark.skipif(
    not get_latex_converter().lualatex_available,
    reason="LuaLaTeX not available"
)
class TestLuaLaTeXPDFGeneration:
    ...
    def test_lualatex_pdf_has_valid_structure(self, latex_converter, sample_latex_with_metadata):
    ...
    def test_lualatex_pdf_has_content_references(self, latex_converter, sample_latex_with_metadata):
...
class TestEndToEndRemediation:
    ...
    def test_remediate_then_convert_produces_valid_pdf(self, sample_latex_without_accessibility):
        ...
        config = RemediationConfig(
            latex_output_formats=['tex', 'pdf']
        )
        remediator = LatexRemediator(sample_latex_without_accessibility, [], config=config)
...
class TestPDFProcessorIntegration:
    ...
    def test_processor_detects_heading_structure(self, sample_latex_with_metadata):
        ...
        # Use the heading detection method directly
        has_h1 = processor._has_h1_in_structure_tree(pdf_path)
...
class TestLaTeXMLPreprocessing:
    ...
    def test_preprocess_removes_document_metadata(self, sample_latex_with_metadata):
        ...
        assert r'\DocumentMetadata{' not in processed, "DocumentMetadata should be removed"
...
class TestToolAvailability:
    ...
    def test_report_tool_status(self, latex_converter):
        ...
        print("="*60)
        print(f"  LuaLaTeX:  {'✓ Available' if latex_converter.lualatex_available else '✗ Not available'}")
        print(f"  LaTeXML:   {'✓ Available' if latex_converter.latexml_available else '✗ Not available'}")
        print(f"  pdflatex:  {'✓ Available' if latex_converter.pdflatex_available else '✗ Not available'}")
        print(f"  pandoc:    {'✓ Available' if latex_converter.pandoc_available else '✗ Not available'}")
        print("="*60)
