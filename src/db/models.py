...
class Department(Base):
    ...
    # Subscription tier: trial, individual_free, department, university
    tier = Column(String(50), default="trial")
    max_users = Column(Integer, default=5)  # User limit based on tier

    # Usage quota tracking (for individual_free tier)
    ...
    # Region/Country for deadline tracking (Phase 4.1)
    ...
    # For pilot/department tiers that need to provide their own AI API keys
    ...
    )  # Manual approval for high-value pilots to use founder's Gemini key
...
class User(Base):
    ...
    lti_source = Column(String(255), nullable=True, index=True)  # Format: {issuer}:{lti_user_id}
    ...
    # Double opt-in confirmation for marketing emails (required for SES production)
...
class WaitlistSignup(Base):
    ...
    )  # Mapped to 'metadata' column
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Double opt-in email confirmation (required for SES production access)
    email_confirmed = Column(Boolean, default=False, nullable=False)
    email_confirmation_token = Column(
        String(64), unique=True, nullable=True, index=True
    )
    email_confirmed_at = Column(DateTime(timezone=True), nullable=True)

    # Email unsubscribe fields (CAN-SPAM / GDPR compliance)
    ...
    # Conversion tracking (waitlist → user)
    converted = Column(Boolean, default=False, nullable=False)
    converted_at = Column(DateTime(timezone=True), nullable=True)
    converted_user_id = Column(String(36), ForeignKey("users.id"), nullable=True)

    # Relationships
    converted_user = relationship(
        "User", backref="waitlist_signup", foreign_keys=[converted_user_id]
    )


class ContactSubmission(Base):
    """Contact form submissions for CRM integration"""
    ...
    source = Column(String(255), nullable=True)  # e.g., "contact-page", "demo-request"
    ...
    crm_person_id = Column(String(36), nullable=True)  # CRM person ID
...
# =============================================================================
# Cloud Integration Models (Phase: Platform Integrations)
# =============================================================================
...
class CampaignStatus(str, Enum):
    """Status of marketing email campaigns"""

    DRAFT = "draft"  # Not yet scheduled
    SCHEDULED = "scheduled"  # Scheduled for future send
    SENDING = "sending"  # Currently being sent
    SENT = "sent"  # Completed sending
    CANCELLED = "cancelled"  # Cancelled before completion


class MarketingCampaign(Base):
    """
    Marketing email campaigns for opt-in users.

    Stores campaign content, scheduling, and aggregate statistics.
    Individual sends are tracked in EmailSend for open/click tracking.
    """
    ...
    preview_text = Column(String(255), nullable=True)  # Preview text in inbox

    # Content
    html_content = Column(Text, nullable=False)  # Full HTML email body
    plain_content = Column(Text, nullable=True)  # Plain text fallback

    # Targeting
    segment = Column(String(50), default="all")  # all, waitlist, pilot, active
    audience_filter = Column(JSON, nullable=True)  # Custom filters

    # Status and scheduling
    status = Column(
        SQLEnum(CampaignStatus, name="campaignstatus"),
    ...
    # Statistics (denormalized for quick dashboard access)
    ...
    created_by = relationship("User", backref="campaigns")

    # Relationships
    sends = relationship("EmailSend", back_populates="campaign", lazy="dynamic")


class EmailSend(Base):
    """
    Individual email send records for tracking.

    Tracks delivery status, opens, and clicks for each recipient.
    Uses unique tracking tokens for open pixel and click redirects.
    """
    ...
    # Campaign reference
    campaign_id = Column(
        String(36), ForeignKey("marketing_campaigns.id"), nullable=False
    )
    ...
    open_count = Column(Integer, default=0)  # Multiple opens tracked
    ...
    user = relationship("User", backref="email_sends")
...
Index("idx_marketing_campaigns_status", MarketingCampaign.status)
Index("idx_marketing_campaigns_scheduled_at", MarketingCampaign.scheduled_at)
Index(
...
# Email send indexes
Index("idx_email_sends_campaign_id", EmailSend.campaign_id)
Index("idx_email_sends_tracking_token", EmailSend.tracking_token)
Index("idx_email_sends_recipient_email", EmailSend.recipient_email)
Index("idx_email_sends_ses_message_id", EmailSend.ses_message_id)
Index(
    "idx_email_sends_campaign_sent",
    EmailSend.campaign_id,
    EmailSend.sent_at.desc(),
)
