...
async def _download_cloud_file(
    ...
    try:
        ...
        if credential.provider == CloudProvider.GOOGLE.value:
            ...
            blackboard_instance_url = credential.provider_metadata.get("blackboard_instance_url")
...
async def _send_remediation_notification(
    ...
    try:
        ...
        if result.failed_count == 0 and result.fixed_count > 0:
            ...
            # Partial success
            # TODO: Use partial success template when available
            logger.info(
                f"Partial success notification needed for scan {scan.id} "
                f"(fixed={result.fixed_count}, failed={result.failed_count})"
            )

        elif result.failed_count > 0 and result.fixed_count == 0:
            # Complete failure
            # TODO: Use failure template when available
            logger.info(
                f"Failure notification needed for scan {scan.id} "
