    ...
    def generate_website_report(scan_data: Dict[str, Any]) -> bytes:
        ...
        if not issues:
            no_issues_text = Paragraph(
                "<b>No findings were recorded by the configured checks.</b><br/>This result is bounded to the completed scan and does not determine whether the document meets an accessibility standard.",
                ParagraphStyle(
                    "NoIssues",
        ...
        publisher = settings.brand_name.strip() if settings.brand_name else ""
        footer_copy = "Automated accessibility scan evidence"
        if publisher:
            footer_copy = (
                f"Report generated by {html.escape(publisher)}<br/>{footer_copy}"
            )
