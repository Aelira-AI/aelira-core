...
to track WCAG 2.1 compliance across all scanned files.
...
class ComplianceDashboard:
    ...
    def get_department_compliance(db: Session, department_id: str) -> ComplianceStats:
        ...
        active_faculty_ids = set(
            scan.user_id for scan in scans if scan.user_id is not None
        )
    ...
    def get_priority_issues(
        ...
        for scan in scans:
            ...
            user = users_by_id.get(scan.user_id) if scan.user_id else None
            user_name = user.name if user else "System scan"
