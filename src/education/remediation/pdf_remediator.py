...
from .table_tagger import TableTagger, TableTagResult
...
class PdfRemediator(BaseRemediator):
        ...
        try:
            ...
            for issue in issues_by_category.get(IssueCategory.LANGUAGE, []):
                try:
                    ...
                    self.result.failed_issues.append(
                        {"issue_id": issue.id, "description": issue.description, "error": str(e)}
                    )
            ...
            for issue in issues_by_category.get(IssueCategory.NAVIGATION, []):
                try:
                    ...
                    logger.error("Error processing NAVIGATION issue %s: %s", issue.id, e)
                    self.result.failed_issues.append(
                        {"issue_id": issue.id, "description": issue.description, "error": str(e)}
                    )
                    self.result.failed_count += 1
            ...
            if specialist_issues.get("role_mapping"):
                self._run_specialist("role_mapping", specialist_issues["role_mapping"], document)
            ...
            if issues_by_category.get(IssueCategory.FORM):
                self._run_specialist("form", issues_by_category[IssueCategory.FORM], document)

            # 10. Links (specialist)
            if issues_by_category.get(IssueCategory.LINK):
                self._run_specialist("link", issues_by_category[IssueCategory.LINK], document)
            ...
            if specialist_issues.get("font_unicode"):
                self._run_specialist("font_unicode", specialist_issues["font_unicode"], document)
            ...
            for issue in issues_by_category.get(IssueCategory.READING_ORDER, []):
                try:
                    ...
                    logger.error("Error processing READING_ORDER issue %s: %s", issue.id, e)
                    self.result.failed_issues.append(
                        {"issue_id": issue.id, "description": issue.description, "error": str(e)}
                    )
                    self.result.failed_count += 1
    ...
    def _run_specialist(
        self, name: str, issues: List[RemediationIssue], document: Any
    ):
            ...
            if name == "form":
                ...
                for i, result in enumerate(results):
                    ...
                    if result.success:
                        ...
                        self._add_fixed_issue(
                            issue,
                            fixed_content=result.notes or f"Fixed {result.links_fixed} links",
                ...
                for i, result in enumerate(results):
                    ...
                    if result.success:
                        self._structure_modified = True
                        desc = result.aria_label or f"Math formula on page {result.page_number}"
                        self._add_fixed_issue(
                            issue,
                            fixed_content=desc,
    ...
    def _compute_fix_metadata(
        self, issue: RemediationIssue, fix_content: str
    ) -> tuple:
        ...
        if self._structure_modified and self._pikepdf_doc:
            ...
            try:
                ...
                try:
                    ...
                    logger.info("Content streams tagged with BDC/EMC markers (v2)")
                except Exception as e:
                    ...
                    try:
                        ...
                        logger.info("Content streams tagged with BDC/EMC markers (v1 fallback)")
                    except Exception as e2:
                        logger.warning(f"Content stream tagging failed (non-fatal): {e2}")
    ...
    def _apply_reading_order_fix(
        ...
        try:
            ...
            if not result.success:
                logger.error(
                    "Reading order fix failed: %s", result.error
                )
                return False
            ...
            if not getattr(self, "_table_tagger_tried", False):
                ...
                if self._structure_modified and self._pikepdf_doc:
                    try:
                        ...
                        logger.debug("Flushed pending pikepdf changes before table tagging")
                    except Exception as e:
                        logger.warning("Could not flush pikepdf changes before table tagging: %s", e)
                ...
                if result.success and result.tables_tagged > 0:
                    ...
                    logger.info(
                        ...
                        result.tables_tagged, result.tables_found,
                        result.total_cells, result.header_cells, result.confidence,
                    )
    ...
    def _add_table_from_metadata(self, issue: RemediationIssue) -> bool:
        ...
        if success:
            ...
            logger.info(
                ...
                page_num, len(headers), len(data_rows),
            )
    ...
    def _apply_list_fix(
        ...
        try:
            ...
            with fitz.open(self.file_path) as doc:
                ...
                if lists_added > 0:
                    self._structure_modified = True
                    logger.info("List structure fix applied: added %d lists", lists_added)
                    return True
                else:
                    logger.warning(
...
Generate only the alt text, nothing else:"""
        ...
        return f"Visual content on page {page_num}"
    ...
    def _calculate_scores(self):
        ...
        if self.result.total_issues > 0:
            ...
            self.result.original_compliance_score = get_score_only(
                original_issue_dicts
            )
            ...
            if (
                ...
                fixed_issue_ids = {
                    f.issue_id for f in self.result.fixed_issues
                }
    ...
    def _verify_fixes(self, output_path: str):
        ...
        try:
            ...
            # Track issue IDs
            original_issue_ids = {issue.id for issue in self.issues}
            original_issue_types = {
                (issue.category.value, issue.location or "", issue.description[:50])
                for issue in self.issues
            }
            ...
            try:
                from ..validation.matterhorn import MatterhornValidator, CheckpointStatus
                mh_validator = MatterhornValidator()
                mh_result = mh_validator.validate(output_path)
                if mh_result:
                    failed_checks = [
                        cp for cp in mh_result.checkpoints
