...
class HeuristicStrategy:
    ...
    def fix(self, pdf_path: str) -> ReadingOrderFixResult:
        ...
        try:
            ...
            reorder_map = {old_idx: new_pos for new_pos, old_idx in enumerate(new_order)}
    ...
    def _detect_headers_footers(self, blocks: List[ContentBlock]) -> None:
        ...
        # --- Header detection ---
        self._detect_repeating_zone(
            blocks, total_pages, header_threshold, is_top=True
        )

        # --- Footer detection ---
        self._detect_repeating_zone(
            blocks, total_pages, footer_threshold, is_top=False
        )

        # --- Page number detection ---
        self._detect_page_numbers(blocks, total_pages, header_threshold, footer_threshold)
    ...
    def _detect_page_numbers(
        ...
        values = [(pg, int(best_per_page[pg].text.strip())) for pg in sorted(best_per_page.keys())]
    ...
    def _compute_reading_order(
        ...
        """
        headers = [b for b in blocks if b.is_header or (b.is_page_number and self._is_top(b, blocks))]
        footers = [b for b in blocks if b.is_footer or (b.is_page_number and not self._is_top(b, blocks))]
        content = [
            b
            for b in blocks
            if not b.is_header and not b.is_footer and not b.is_page_number
    ...
    def _reorder_structure_tree(
        ...
        with pikepdf.open(pdf_path, allow_overwriting_input=True) as pdf:
            ...
            for b in blocks:
                if (b.is_header or b.is_footer or b.is_page_number) and b.index < num_kids:
...
class VisionStrategy:
    ...
    def _parse_ai_response(
        self, content: str, num_blocks: int
    ) -> Optional[List[int]]:
