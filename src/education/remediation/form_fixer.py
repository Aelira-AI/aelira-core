...
class FormFixer:
    ...
    def fix(self, issues: List[RemediationIssue]) -> List[FixResult]:
        ...
        for issue in issues:
            ...
            try:
                ...
                logger.error(
                    "FormFixer.fix error for issue %s: %s", issue.id, exc, exc_info=True
                )
                result = FixResult(
                    issue_id=issue.id,
                    success=False,
        ...
        for field_ref in fields_array:
            try:
                ...
                if target_page_index is not None:
                    ...
                    if (
                        field_page_index is not None
                        and field_page_index != target_page_index
                    ):
                        continue
    ...
    def _infer_label(self, field, page_index: Optional[int]) -> Tuple[str, float]:
    ...
    def _get_field_rect(self, field) -> Optional[Tuple[float, float, float, float]]:
