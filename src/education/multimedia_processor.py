...
class MultimediaProcessor:
    ...
    def _enhance_captions_with_visual_context(
        ...
        speaker_appearances = self._extract_speaker_info_from_descriptions(
            audio_descriptions
        )
        ...
        for segment in transcription:
            ...
            if segment.is_sound_effect and context:
                ...
                if visual_cue:
                    enhanced_text = f"[{visual_cue}] {enhanced_text}".replace(
                        "[[", "["
                    ).replace("]]", "]")
    ...
    def _extract_visual_cue(self, context: Dict) -> Optional[str]:
        ...
        visual_cue_patterns = [
            ...
            (
                r"(?:shows?|displays?)\s+(?:the\s+)?text[:\s]+[\"']?(.+?)[\"']?(?:\.|$)",
                "Text",
            ),
        ]
    ...
    def _check_frame_red_saturation(self, frame_path: str) -> Tuple[bool, float]:
        ...
        try:
            ...
            with Image.open(frame_path) as img:
                # Convert to RGB if necessary (new name: ImageFile vs Image
                # differ under Pillow 12.3 typing, so keep a separate var)
                rgb_img: Image.Image = img.convert("RGB") if img.mode != "RGB" else img

                # Convert to numpy array
                pixels = np.array(rgb_img, dtype=np.float32)
    ...
    def _extract_keyframes(
        ...
        try:
            ...
            if len(merged_timestamps) > MAX_FRAMES:
                ...
                merged_timestamps = [
                    merged_timestamps[int(i * step)] for i in range(MAX_FRAMES)
                ]
    ...
    def _detect_scene_changes_pyscenedetect(self, video_path: str) -> List[float]:
        """
    ...
    def _detect_scene_changes(self, video_path: str, duration: float) -> List[float]:
        ...
        try:
            ...
            logger.warning(
                "[MultimediaProcessor] Scene detection timed out, using fallback"
            )
    ...
    def create_accessible_video(
        ...
        try:
            ...
            if audio_description_path:
                ...
                output_ext = (
                    input_ext if input_ext in [".mp4", ".mkv", ".webm"] else ".mp4"
                )
            ...
            if audio_desc_input is not None:
                cmd.extend(
                    ["-map", f"{audio_desc_input}:a"]
                )  # Audio descriptions as second audio track
            ...
            logger.info(
                f"[MultimediaProcessor] Creating accessible video: {' '.join(cmd)}"
            )
            ...
            if os.path.exists(output_path):
                logger.info(
                    f"[MultimediaProcessor] Created accessible video: {output_path}"
                )
                return output_path
            else:
            ...
            logger.error(
                f"[MultimediaProcessor] Failed to create accessible video: {e}"
            )
            return None
