...
class MatterhornValidator:
    ...
    def validate(
        ...
        return MatterhornResult(checkpoints=checkpoints, ua_version=resolved_version)
        ...
        if invalid_mappings:
            return [
                MatterhornCheckpoint(
                    ...
                    status=CheckpointStatus.FAIL,
                    severity="error",
                    details=(f"Invalid role mappings: {', '.join(invalid_mappings)}"),
                )
            ]
    ...
    def _check_content_marking(self, pdf: Any) -> List[MatterhornCheckpoint]:
        ...
        try:
            for page in pdf.pages:
                ...
                has_marking = any(str(op.operator) in ("BDC", "BMC") for op in ops)
                if has_marking:
                    return [
                        MatterhornCheckpoint(
                            id="01-001",
    ...
    def _resolve_ua_version(self, pdf: Any, ua_version: Union[int, str]) -> int:
        """
    ...
    def _detect_ua_version(self, pdf: Any) -> int:
        ...
        try:
            with pdf.open_metadata() as meta:
                ...
                # or pdfuaid:part="2"
                match = re.search(r"pdfuaid:part[\">\s]*(\d+)", xmp_raw)
                if match:
    ...
    def _check_ua2_ruby(self, pdf: Any) -> List[MatterhornCheckpoint]:
        ...
        for i, ruby in enumerate(ruby_elements, 1):
            ...
            if not has_rb or not has_rt:
                ...
                if not has_rt:
                    missing.append("RT")
                invalid_rubies.append(f"Ruby {i}: missing {', '.join(missing)}")
    ...
    def _find_elements_by_type(self, pdf: Any, element_type: str) -> List[Any]:
        """
    ...
    def _walk_tree(self, element: Any, target_type: str, results: List[Any]) -> None:
    ...
    def _find_elements_with_key(self, pdf: Any, key: str) -> List[Any]:
        """
