...
class GeminiProvider(LLMProvider):
    ...
    async def _call_api(
        ...
        # initialize() refuses to start without a key, but that is not visible
        # to callers of this method — state it here so an unconfigured provider
        # fails with its own message rather than a 400 from Google.
        if not self.api_key:
            return "ERROR: Gemini API key not configured", 0.0
