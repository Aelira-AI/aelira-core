...
class IndividualSignupRequest(BaseModel):
    ...
    def validate_edu_email(cls, v):
        ...
        # Block disposable/temp-mail providers that exploit openly-registrable
        # academic ccTLDs (e.g. *.edu.pl) to pass the structural .edu check.
...
async def request_magic_link(
    ...
    # For NEW signups (no existing account), run the abuse detector.
    # This is the same gate as /auth/signup/individual, which the magic-link
    # path previously bypassed entirely (free-tier abuse, 2026-06-03).
