...
def _normalize_issues_for_remediation(issues: list) -> list:
    ...
    Mirrors the logic in ``demo_routes._normalize_issue()``.
    """
    normalized = []
    for i, issue in enumerate(issues):
        ...
        # Scanner uses 1-based slide_number; remediator needs 0-based slide_index
...
async def remediate_scan(
    ...
    if not file_path or not os.path.exists(file_path):
        ...
        cloud_file = (
            db.query(CloudFile)
            .filter(CloudFile.last_scan_id == scan_id)
            .first()
        )
        if cloud_file and cloud_file.credential_id:
            ...
            if credential:
                try:
                    ...
                    ext = cloud_file.file_type or "pdf"
                    local_path = os.path.join(temp_dir, f"{cloud_file.file_name or 'file'}")

                    if credential.provider == CloudProvider.CANVAS.value:
                        from ...integrations.canvas.canvas_api import CanvasAPIClient

                        canvas_url = (credential.provider_metadata or {}).get("canvas_instance_url", "")
                        if os.getenv("ENV") == "development" and "localhost" in canvas_url:
                            canvas_url = canvas_url.replace("localhost", "host.docker.internal")

                        client = CanvasAPIClient(canvas_instance_url=canvas_url, access_token=access_token)
                        try:
                            dl_result = await client.download_file(
                                file_id=cloud_file.provider_file_id, local_path=local_path
                            )
                            if dl_result.success:
                                ...
                                db.commit()
                                logger.info(f"Re-downloaded Canvas file for remediation: {cloud_file.file_name}")
                        finally:
                            await client.close()
                        ...
                        drive = GoogleDriveIntegration(credential_id=credential.id, access_token=access_token)
                        try:
                            ...
                            if dl_result.success:
                                ...
                                scan.storage_path = file_path
                                db.commit()
                                logger.info(f"Re-downloaded Google Drive file for remediation: {cloud_file.file_name}")
                        finally:
                            await drive.close()

                except Exception as e:
                    logger.error(f"Failed to re-download cloud file for remediation: {e}")
    ...
    # This mirrors the normalization in demo_routes._normalize_issue().
        ...
        for fix in result.fixed_issues:
            db.add(
                ScanFix(
                    ...
                    original_content=_sanitize_str(getattr(fix, "original_content", None)),
...
async def batch_remediate(
    ...
    REQUIRES API KEY IN PRODUCTION.
    REQUIRES: bulk_api feature (not available on free/plus/trial tiers)
    """
    _, user_id, department_id = api_key_info
