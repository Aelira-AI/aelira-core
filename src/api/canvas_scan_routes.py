...
def _rewrite_localhost_for_docker(api_client: CanvasAPIClient) -> None:
    ...
    if (
        ...
        and "localhost" in api_client.canvas_url
    ):
...
async def _canvas_scan_file_task(
...
async def scan_canvas_file(
    ...
    try:
        credential, api_client = await _get_canvas_client(
            dept_id, db
        )
        ...
        try:
            ...
            if existing_cloud_file:
                ...
                existing_cloud_file.file_name = file_info.display_name or file_info.filename
...
async def get_course_scan_status(
    course_id: str,
    file_ids: str = Query(
        ..., description="Comma-separated Canvas file IDs"
    ),
    db: Session = Depends(get_db_dependency),
    api_key_info: Tuple[Optional[APIKey], str, str] = Depends(get_required_api_key),
    ...
    for file_id in requested_file_ids:
        ...
        if cf.last_scan_id:
            ...
            if scan:
                status = (str(scan.status.value) if hasattr(scan.status, "value") else str(scan.status)).lower()
                scan_result = (
                    db.query(ScanResult)
    ...
    average_compliance = (
        round(total_compliance / scanned_count, 1)
        if scanned_count > 0
        else None
    )
...
async def upload_remediated_to_canvas(
    ...
    try:
        ...
        logger.error(
            f"Failed to upload remediated file to Canvas: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500,
