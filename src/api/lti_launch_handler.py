...
Flow:
  Canvas → POST /lti/launch → handle_lti_launch()
      ...
      ↳ 302 → dashboard.aelira.ai/lti?code=...
...
def handle_lti_launch(
    ...
    payload = json.dumps(
        {"access_token": token, "course_id": course_id, "course_name": course_name}
    )
    _store_code(f"lti_code:{code}", payload, ttl=120)

    # --- Build redirect URL ---
    dashboard_url = (
        getattr(settings, "dashboard_url", None) or "https://dashboard.aelira.ai"
    )
