...
class EmailService:
    ...
    def __init__(
        ...
        self.from_email = from_email or os.getenv("FROM_EMAIL", "noreply@aelira.ai")
        ...
        html = self.render_template(
            "weekly_summary",
            {
                ...
                <p style="color: #666; font-size: 14px;">
                    Remember: The WCAG 2.1 compliance deadline is April 24, 2026.
                </p>
            """,
            },
        )
    ...
    async def send_welcome_individual(
        self,
        to_email: str,
        name: str,
        api_key: str,
        tier: str = "individual_free",
        scans_limit: int = 10,
        pages_limit: int = 50,
    ) -> Dict[str, Any]:
        """
        Send welcome email to new individual faculty signup.

        Args:
            to_email: User's email address
            name: User's name
            api_key: The generated API key (shown only once)
            tier: The user's tier (for future use)
            scans_limit: Documents per month limit
            pages_limit: Pages per document limit
        ...
        return await self.send_email(
            to_emails=[to_email],
            subject=f"Welcome to Aelira, {name}! Your Free Account is Ready",
            html_content=html,
        )
    ...
    async def send_welcome_magic_link(
        ...
        Unlike send_welcome_individual, this doesn't include an API key
        ...
        Returns:
            Dict with success status
        """
        # Determine tier display name and limits
        tier_info = {
            "individual_free": {
                "display_name": "Free",
                "scans_limit": 10,
        ...
        }.get(tier, {"display_name": "Free", "scans_limit": 10, "pages_limit": 50})
        ...
        content = f"""
            ...
            <div style="text-align: center; margin: 32px 0;">
                <a href="https://dashboard.aelira.ai/dashboard"
                   style="background-color: #7C3AED; background: linear-gradient(135deg, #8B5CF6 0%, #6366F1 100%); color: white; padding: 16px 32px;
                          text-decoration: none; border-radius: 8px; display: inline-block;
                          font-weight: 600; font-size: 16px;">
                    Go to Dashboard
            ...
            <div style="background: #fffbeb; border-radius: 8px; padding: 16px; margin-top: 24px;">
                <p style="color: #92400e; margin: 0; font-size: 14px;">
                    ...
                    for educational institutions is April 24, 2026. Start scanning your
        ...
        return await self.send_email(
            ...
            html_content=html,
            text_content=f"Welcome to Aelira, {name}!\n\nYour account is now active. Visit https://dashboard.aelira.ai to start scanning your documents for accessibility compliance.\n\nPlan: {tier_info['display_name']}\n- {tier_info['scans_limit']} document scans per month\n- Up to {tier_info['pages_limit']} pages per document\n\nThe WCAG 2.1 compliance deadline is April 24, 2026.\n\nNeed help? Contact support@aelira.ai",
        )
...
Aelira is an accessibility compliance platform that helps you make your course materials WCAG 2.1 compliant before the April 24, 2026 deadline.

If you weren't expecting this invitation, you can safely ignore this email.

Questions? Contact support@aelira.ai""",
        )

    async def send_pilot_welcome(
        self,
        to_email: str,
        recipient_name: str,
        institution_name: str,
        pilot_count: int = 10,
        dashboard_url: str = "https://dashboard.aelira.ai",
    ) -> Dict[str, Any]:
        """
        Send pilot program welcome email.

        Args:
            to_email: Admin's email address
            recipient_name: Name of the recipient (usually department admin)
            institution_name: Name of the institution/department
            pilot_count: Current number of pilot participants
        ...
        # NOTE: This is a TRANSACTIONAL email (account confirmation).
        return await self.send_email(
            to_emails=[to_email],
            subject=f"Aelira Pilot Program - Account Confirmed ({institution_name})",
            html_content=html,
            text_content=f"""Aelira Pilot Program - Account Confirmed

Dear {recipient_name},

Your pilot program account for {institution_name} is now active.
You are participant #{pilot_count} in the pilot program.
...
NEXT STEPS:
1. Log in to your dashboard using this email address
2. Configure your AI provider in Settings
3. Upload your first document to begin scanning

WCAG 2.1 AA DEADLINE: April 24, 2026
Title II compliance deadline for universities.

Need help? Contact pilot@aelira.ai
Documentation: https://aelira.ai/docs

--
Aelira
AI-Powered Accessibility for Higher Education

This is a transactional email confirming your pilot program enrollment.""",
        )
    ...
    async def send_email_confirmation(
        ...
        context: str = "newsletter",
    ) -> Dict[str, Any]:
        """
        Send double opt-in email confirmation.

        This is a TRANSACTIONAL email — it confirms the user's intent to
        receive emails. Required for AWS SES production access.

        Args:
            to_email: Email address to confirm
            confirmation_url: Full URL to confirm the email
            context: What they're confirming ("newsletter" or "marketing")

        Returns:
            Dict with success status
        """
        if context == "marketing":
            purpose_text = "marketing and product update emails"
            subject = "Confirm your Aelira email preferences"
        else:
            purpose_text = "newsletter and product updates"
            subject = "Confirm your email address — Aelira"

        content = f"""
            <h2 style="color: #1a1a2e; margin: 0 0 16px 0;">Confirm Your Email</h2>

            <p>
                Please confirm your email address to start receiving {purpose_text}
                from Aelira.
            </p>
            ...
            <p style="background: #f3f4f6; padding: 12px; border-radius: 6px; word-break: break-all;
                      font-size: 12px; color: #333;">
                {html_lib.escape(confirmation_url)}
            </p>

            <div style="margin-top: 24px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                <p style="color: #6b7280; font-size: 12px; margin: 0;">
                    <strong>Didn't sign up?</strong> You can safely ignore this email.
                    No emails will be sent unless you confirm.
                </p>
            </div>
        """
        html = get_email_wrapper(content)
        ...
        return await self.send_email(
            ...
            subject=subject,
            html_content=html,
            text_content=f"""Confirm Your Email Address

Please confirm your email to start receiving {purpose_text} from Aelira.

Click this link to confirm: {confirmation_url}

Didn't sign up? You can safely ignore this email. No emails will be sent unless you confirm.
...
https://aelira.ai""",
    ...
    async def send_waitlist_confirmation(
        ...
        Args:
            to_email: Email address of the signup
            unsubscribe_url: Optional unsubscribe URL

        Returns:
            Dict with success status
        """
        html = render_waitlist_confirmation_email(unsubscribe_url=unsubscribe_url)

        return await self.send_email(
            to_emails=[to_email],
            subject="You're on the Aelira waitlist",
            html_content=html,
            text_content="""You're on the Aelira Waitlist

Thanks for joining the Aelira waitlist. We'll notify you when your account is ready.

Questions? Contact hello@aelira.ai

--
Aelira
https://aelira.ai

This email confirms your waitlist signup.""",
        )
