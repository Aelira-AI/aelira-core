...
This module provides semantic search over WCAG guidelines using pgvector.
It retrieves relevant guidelines based on violation context to ground LLM
classification in canonical WCAG definitions.
...
- Grounds LLM outputs in factual WCAG reference material

Expected Results:
- 95-99% consistency (vs current 40%) in severity classification
- Canonical severity criteria from WCAG knowledge base
- Reduced hallucination and inconsistent classifications
"""
...
class WCAGKnowledgeBase:
    """
    WCAG Knowledge Base with semantic search via pgvector.
    ...
    async def search(
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                logger.info(
                    f"Found {len(results)} guidelines for query: {query[:50]}..."
                )
                return results
