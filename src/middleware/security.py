...
class CSRFMiddleware(BaseHTTPMiddleware):
    ...
    EXEMPT_PATHS = [
        # ── Dashboard API endpoints (auth required, SameSite cookies) ──
        "/education/",  # scan, remediate, image, multimedia, code, etc.
        "/auth/keys",  # API key CRUD
        "/auth/profile",  # profile updates
        "/auth/session/logout",  # logout
        "/auth/sessions",  # session management
        "/alerts/",  # alert settings
        "/analytics/",  # snapshots, issues, team features
        "/llm/",  # LLM provider management
        "/tts/",  # text-to-speech
        "/integrations/",  # cloud integration management
        "/billing/",  # checkout, portal, webhook
        "/admin/",  # admin/campaign routes
        ...
        "/demo-request",
        "/enterprise-inquiry",
        # Account creation (public, rate-limited)
        ...
        # Brightspace REST API (dashboard SPA, session cookies + SameSite)
        "/brightspace/connect",
