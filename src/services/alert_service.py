...
class AlertService:
    ...
    async def send_scan_complete_alert(
        ...
        try:
            ...
            logger.error(f"SMTP error sending scan complete alert: {e}")
            return False
        except (OSError, ConnectionError) as e:
            logger.error(f"Connection error sending scan complete alert: {e}")
            return False
        except Exception as e:
            # This is a notification path that must never crash the caller
            # (remediation/scan jobs shouldn't fail because an email
            # couldn't be sent), so an unanticipated exception is still
            # caught here -- just logged loud, with a traceback.
            logger.error(
                f"Unexpected error sending scan complete alert: {e}", exc_info=True
            )
            return False
    ...
    async def send_critical_issue_alert(
        ...
        try:
            ...
            logger.error(f"SMTP error sending critical issue alert: {e}")
            return False
        except (OSError, ConnectionError) as e:
            logger.error(f"Connection error sending critical issue alert: {e}")
            return False
        except Exception as e:
            # Notification path that must never crash the caller -- see
            # the equivalent comment in send_scan_complete_alert above.
            logger.error(
                f"Unexpected error sending critical issue alert: {e}", exc_info=True
            )
            return False
    ...
    async def send_weekly_summary(
        ...
        try:
            ...
            logger.error(f"Database error sending weekly summary: {e}")
            return False
        except smtplib.SMTPException as e:
            logger.error(f"SMTP error sending weekly summary: {e}")
            return False
        except (OSError, ConnectionError) as e:
            logger.error(f"Connection error sending weekly summary: {e}")
            return False
        except Exception as e:
            # Notification path that must never crash the caller -- see
            # the equivalent comment in send_scan_complete_alert above.
            logger.error(f"Unexpected error sending weekly summary: {e}", exc_info=True)
            return False

    async def send_remediation_partial_success_alert(
        self,
        to_emails: List[str],
        scan_id: str,
        file_name: str,
        ...
        """
        Send partial-success remediation alert: some issues were fixed
        automatically, some could not be.

        Uses the same ALERT_REMEDIATION_COMPLETE preference as a full
        success (email_remediation_complete) since both are "remediation
        finished, here's what happened" notifications.
        ...
        Args:
            ...
            failed_count: Number of issues that could not be fixed
            manual_count: Number of issues still needing manual review
            fixed_issues: Optional list of {"description": ...} dicts
            failed_issues: Optional list of {"description": ..., "error": ...} dicts
            scan_url: URL to view scan results
            department_id: Department ID (for checking alert settings)
            db: Database session (for checking alert settings)
        ...
        if db and department_id:
            if not self.check_department_alert_enabled(
                department_id, ALERT_REMEDIATION_COMPLETE, db
            ):
                logger.info(
                    f"Remediation alerts disabled for department {department_id}"
                )
                return False

        if db:
            filtered_emails = self.filter_emails_by_preference(
                to_emails, ALERT_REMEDIATION_COMPLETE, db
            )
            if not filtered_emails:
                logger.info("No recipients have remediation alerts enabled")
                return True
            to_emails = filtered_emails

        try:
            result = await self.email_service.send_remediation_partial_success(
                to_emails=to_emails,
                file_name=file_name,
            ...
            if result.get("success"):
                logger.info(
                    f"Partial success remediation alert sent for scan {scan_id} "
                    f"to {len(to_emails)} recipients"
                )
                return True
            else:
                logger.error(
                    f"Failed to send partial success remediation alert: "
                    f"{result.get('error')}"
                )
                return False

        except smtplib.SMTPException as e:
            logger.error(f"SMTP error sending partial success remediation alert: {e}")
            return False
        except (OSError, ConnectionError) as e:
            logger.error(
                f"Connection error sending partial success remediation alert: {e}"
            )
            return False
        except Exception as e:
            # Notification path that must never crash the caller -- see
            # the equivalent comment in send_scan_complete_alert above.
            logger.error(
                f"Unexpected error sending partial success remediation alert: {e}",
                exc_info=True,
            )
            return False

    async def send_remediation_failure_alert(
        self,
        to_emails: List[str],
        scan_id: str,
        file_name: str,
        ...
        """
        Send failure remediation alert: automatic remediation could not fix
        ...
        try:
            result = await self.email_service.send_remediation_failure(
                to_emails=to_emails,
                file_name=file_name,
            ...
            if result.get("success"):
                logger.info(
                    f"Failure remediation alert sent for scan {scan_id} "
                    f"to {len(to_emails)} recipients"
                )
                return True
            else:
                logger.error(
                    f"Failed to send failure remediation alert: {result.get('error')}"
                )
                return False

        except smtplib.SMTPException as e:
            logger.error(f"SMTP error sending failure remediation alert: {e}")
            return False
        except (OSError, ConnectionError) as e:
            logger.error(f"Connection error sending failure remediation alert: {e}")
            return False
        except Exception as e:
            # Notification path that must never crash the caller -- see
            # the equivalent comment in send_scan_complete_alert above.
            logger.error(
                f"Unexpected error sending failure remediation alert: {e}",
                exc_info=True,
            )
            return False
