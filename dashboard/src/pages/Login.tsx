    ...
    if (errorParam) {
      switch (errorParam) {
        ...
        case 'token_error':
          ...
          break;
        case 'tier_required':
          setError(messageParam || 'OAuth login is only available for department and university tier accounts.');
          break;
        case 'no_email':
            ...
            <>
              ...
              {oauthStatus && !oauthStatus.oauth_allowed && email.includes('@') && !checkingOauth && (
                <p className="text-xs text-tertiary mt-4 text-center">
                  OAuth login is available for department and university accounts.{' '}
                  <a href="https://example.com/pricing" className="text-accent hover:underline">
                    Upgrade your plan
                  </a>{' '}
                  to unlock SSO.
                </p>
