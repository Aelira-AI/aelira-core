...
interface EmailPreferences {
  ...
  email_marketing_pending_confirmation?: boolean;
    ...
    <div className="card">
      ...
      <div className="px-6 py-4 space-y-4">
        ...
        >
          ...
          <div className="space-y-3">
            <PreferenceItem
              icon={<Megaphone className="w-5 h-5" style={{ color: 'var(--content-warning)' }} />}
              label="Product News & Updates"
              description="Feature announcements, tips, and accessibility news"
              enabled={preferences.email_marketing}
              onChange={(v) => updatePreference('email_marketing', v)}
            />
            {preferences.email_marketing_pending_confirmation && (
              <div
                className="flex items-center gap-2 px-3 py-2 rounded-md text-xs"
                style={{ backgroundColor: 'var(--feature-warning-surface)', color: 'var(--feature-warning-content)' }}
              >
                <Mail className="w-4 h-4 shrink-0" />
                <span>Check your inbox — a confirmation email has been sent. You must confirm to receive marketing emails.</span>
              </div>
            )}
          </div>
        </div>
