...
def upgrade() -> None:
    ...
    op.add_column(
        "waitlist_signups", sa.Column("converted_user_id", sa.String(36), nullable=True)
    )
    ...
    op.create_index("idx_waitlist_signups_converted", "waitlist_signups", ["converted"])
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_waitlist_converted_user", "waitlist_signups", type_="foreignkey"
    )
