...
def upgrade() -> None:
    ...
    op.add_column('scan_results', sa.Column('scan_mode', sa.Enum('quick', 'comprehensive', 'deep', name='scanmode'), nullable=True))

    # Add separate result columns for each engine
    op.add_column('scan_results', sa.Column('axe_results', sa.JSON(), nullable=True,
        comment='Raw axe-core results (violations array)'))
    op.add_column('scan_results', sa.Column('pa11y_results', sa.JSON(), nullable=True,
        comment='Raw Pa11y results (issues array)'))
    op.add_column('scan_results', sa.Column('ai_vision_results', sa.JSON(), nullable=True,
        comment='AI vision analysis results (for deep mode)'))

    # Add merged results (deduplicated with engine attribution)
    op.add_column('scan_results', sa.Column('merged_results', sa.JSON(), nullable=True,
        comment='Deduplicated results with detected_by attribution'))

    # Add engine usage tracking
    op.add_column('scan_results', sa.Column('engines_used', sa.JSON(), nullable=True,
        comment='Array of engine names used: ["axe-core", "pa11y"]'))

    # Add engine-specific issue counts
    op.add_column('scan_results', sa.Column('axe_issues', sa.Integer(), nullable=True,
        comment='Number of issues found by axe-core'))
    op.add_column('scan_results', sa.Column('pa11y_issues', sa.Integer(), nullable=True,
        comment='Number of issues found by Pa11y'))
    op.add_column('scan_results', sa.Column('issues_found_by_both', sa.Integer(), nullable=True,
        comment='Number of duplicate issues found by both engines'))
    op.add_column('scan_results', sa.Column('unique_issues', sa.Integer(), nullable=True,
        comment='Number of unique issues after deduplication'))

    # Add coverage metrics
    op.add_column('scan_results', sa.Column('estimated_coverage_pct', sa.Float(), nullable=True,
        comment='Estimated WCAG coverage percentage based on engines used'))

    # Add scan duration tracking per engine
    op.add_column('scan_results', sa.Column('axe_duration_ms', sa.Integer(), nullable=True,
        comment='Time taken for axe-core scan in milliseconds'))
    op.add_column('scan_results', sa.Column('pa11y_duration_ms', sa.Integer(), nullable=True,
