...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "campaign_id",
            sa.String(36),
            sa.ForeignKey("marketing_campaigns.id"),
            nullable=False,
        ),
    ...
    op.create_index("idx_marketing_campaigns_status", "marketing_campaigns", ["status"])
    op.create_index(
        "idx_marketing_campaigns_scheduled_at", "marketing_campaigns", ["scheduled_at"]
    )
    op.create_index(
    ...
    op.create_index("idx_email_sends_campaign_id", "email_sends", ["campaign_id"])
    op.create_index("idx_email_sends_tracking_token", "email_sends", ["tracking_token"])
    op.create_index(
        "idx_email_sends_recipient_email", "email_sends", ["recipient_email"]
    )
    op.create_index("idx_email_sends_ses_message_id", "email_sends", ["ses_message_id"])
    op.create_index(
