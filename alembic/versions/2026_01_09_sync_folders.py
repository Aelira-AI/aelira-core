...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "provider_folder_id", sa.String(255), nullable=False
        ),  # Drive folder ID / OneDrive folder ID
        ...
        sa.Column(
            "sync_subfolders", sa.Boolean, default=True, nullable=False
        ),  # Sync all subfolders
        sa.Column(
    ...
    # Create indexes
    op.create_index(
        "ix_cloud_sync_folders_department_id", "cloud_sync_folders", ["department_id"]
    )
    op.create_index(
        "ix_cloud_sync_folders_credential_id", "cloud_sync_folders", ["credential_id"]
    )
    op.create_index(
