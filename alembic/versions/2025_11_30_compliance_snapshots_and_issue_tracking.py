...
def upgrade() -> None:
    ...
    issue_priority_enum = postgresql.ENUM(
        ...
        name='issuepriority', create_type=False
    )
    ...
    op.create_table('compliance_snapshots',
        ...
        sa.Column('files_compliant', sa.Integer(), nullable=False, default=0),  # Score >= 90
        sa.Column('files_needs_work', sa.Integer(), nullable=False, default=0),  # Score 70-89
        sa.Column('files_critical', sa.Integer(), nullable=False, default=0),  # Score < 70
        ...
        sa.UniqueConstraint('department_id', 'snapshot_date', name='uq_snapshot_department_date')
    )

    # Create indexes for efficient querying
    op.create_index('idx_snapshots_department_date', 'compliance_snapshots',
                   ['department_id', sa.text('snapshot_date DESC')], unique=False)
    op.create_index('idx_snapshots_date', 'compliance_snapshots',
                   [sa.text('snapshot_date DESC')], unique=False)
    ...
    op.create_table('issue_tracking',
        sa.Column('id', sa.String(length=36), nullable=False),
        sa.Column('scan_id', sa.String(length=36), nullable=False),
        sa.Column('department_id', sa.String(length=36), nullable=False),

        # Issue identification (from scan results)
        sa.Column('issue_hash', sa.String(length=64), nullable=False),  # Unique hash of issue
        sa.Column('issue_type', sa.String(length=100), nullable=False),
        sa.Column('severity', issue_priority_enum, nullable=False),
        sa.Column('wcag_criterion', sa.String(length=20), nullable=True),  # e.g., "1.1.1", "2.1.1"
        sa.Column('description', sa.Text(), nullable=False),
        sa.Column('element_selector', sa.String(length=512), nullable=True),  # CSS selector or location
        ...
        sa.Column('resolution_method', sa.String(length=50), nullable=True),  # 'auto', 'manual', 'wont_fix'

        # Collaboration
        sa.Column('notes', sa.Text(), nullable=True),  # Team notes/discussion
        sa.Column('priority_override', issue_priority_enum, nullable=True),  # Manual priority adjustment
    ...
    op.create_index('idx_issues_department', 'issue_tracking', ['department_id'], unique=False)
    ...
    op.create_index('idx_issues_assigned', 'issue_tracking', ['assigned_to'], unique=False)
    op.create_index('idx_issues_hash', 'issue_tracking', ['issue_hash'], unique=False)
    op.create_index('idx_issues_department_status', 'issue_tracking',
                   ['department_id', 'status'], unique=False)
