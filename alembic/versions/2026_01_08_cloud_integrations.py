...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "department_id", "provider", name="uq_cloud_credentials_dept_provider"
        ),
    )

    # Indexes for cloud_oauth_credentials
    op.create_index(
        "ix_cloud_oauth_credentials_department",
        "cloud_oauth_credentials",
        ["department_id"],
    )
    op.create_index(
        "ix_cloud_oauth_credentials_provider", "cloud_oauth_credentials", ["provider"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "credential_id",
            sa.String(36),
            sa.ForeignKey("cloud_oauth_credentials.id", ondelete="CASCADE"),
            nullable=False,
        ),
        # Provider-specific IDs
        sa.Column("provider", sa.String(20), nullable=False),  # google, microsoft
        sa.Column(
            "provider_file_id", sa.String(255), nullable=False
        ),  # Google Drive ID / OneDrive ID
        sa.Column(
            "provider_parent_id", sa.String(255), nullable=True
        ),  # Folder/drive ID
        # File metadata
        sa.Column("file_name", sa.String(512), nullable=False),
        sa.Column(
            "file_type", sa.String(20), nullable=False
        ),  # docx, pptx, xlsx, pdf, gdoc, gslide, gsheet
        ...
        # Version tracking
        sa.Column(
            "provider_version", sa.String(100), nullable=True
        ),  # etag/version for change detection
        ...
        # Remediation state
        sa.Column(
            "has_remediated_version", sa.Boolean, server_default="false", nullable=False
        ),
        sa.Column(
            "remediated_file_id", sa.String(255), nullable=True
        ),  # ID of fixed file if uploaded
        # Timestamps
        sa.Column(
            "discovered_at",
            sa.DateTime(timezone=True),
        ...
        sa.UniqueConstraint(
            ...
            "provider",
            "provider_file_id",
            name="uq_cloud_files_dept_provider_file",
        ),
    )
    ...
    op.create_index(
        "ix_cloud_files_needs_rescan", "cloud_files", ["department_id", "needs_rescan"]
    )
    op.create_index(
        "ix_cloud_files_provider_file", "cloud_files", ["provider", "provider_file_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column("provider", sa.String(20), nullable=False),
        sa.Column(
            "subscription_id", sa.String(255), nullable=False
        ),  # Provider's subscription/channel ID
        sa.Column(
            "resource_uri", sa.String(1024), nullable=True
        ),  # What we're watching (folder, drive, etc.)
    ...
    op.create_index(
        "ix_cloud_webhooks_credential", "cloud_webhook_subscriptions", ["credential_id"]
    )
    op.create_index(
        "ix_cloud_webhooks_active",
        "cloud_webhook_subscriptions",
        ["provider", "is_active"],
    )
    op.create_index(
        "ix_cloud_webhooks_expiration",
        "cloud_webhook_subscriptions",
        ["expiration_time"],
    )
    ...
    op.create_table(
        ...
        # Alert types
        sa.Column(
            "alert_on_scan_complete", sa.Boolean, server_default="true", nullable=False
        ),
        sa.Column(
            "alert_on_new_issues", sa.Boolean, server_default="true", nullable=False
        ),
        sa.Column(
        ...
        sa.Column(
            "alert_weekly_summary", sa.Boolean, server_default="true", nullable=False
        ),
        # Delivery preferences
        sa.Column(
            "email_addresses", sa.JSON, nullable=True
        ),  # Array of email addresses
        sa.Column(
            "min_severity", sa.String(20), server_default="medium", nullable=False
        ),  # Only alert >= this severity
        # Quiet hours (optional)
        sa.Column("quiet_hours_start", sa.Time, nullable=True),
        sa.Column("quiet_hours_end", sa.Time, nullable=True),
        sa.Column(
            "timezone", sa.String(50), server_default="America/New_York", nullable=False
        ),
        # Timestamps
        sa.Column(
            "created_at",
        ...
        sa.UniqueConstraint(
            "department_id", "user_id", name="uq_email_alerts_dept_user"
        ),
    )

    # Indexes for email_alert_settings
    op.create_index(
        "ix_email_alerts_department", "email_alert_settings", ["department_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_cloud_webhooks_expiration", table_name="cloud_webhook_subscriptions"
    )
    op.drop_index("ix_cloud_webhooks_active", table_name="cloud_webhook_subscriptions")
    op.drop_index(
        "ix_cloud_webhooks_credential", table_name="cloud_webhook_subscriptions"
    )
    ...
    op.drop_index(
        "ix_cloud_oauth_credentials_provider", table_name="cloud_oauth_credentials"
    )
    op.drop_index(
        "ix_cloud_oauth_credentials_department", table_name="cloud_oauth_credentials"
    )
