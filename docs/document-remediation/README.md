...
Choose a format guide: [PDF](pdf.md), [Office: DOCX, PPTX, XLSX](office.md), or [LaTeX](latex.md). The provider-neutral [commutative-diagram contract](commutative-diagrams.md) documents a bounded semantic graph boundary; it does not add image recognition or document mutation. The [handwritten-math suitability corpus](handwritten-math.md) freezes the offline boundary for later HMER work and explicitly does not claim recognition.
...
| Handwritten-math suitability | [classifier and evidence contract](../../src/education/handwritten_math_suitability.py), [frozen corpus manifest](../../tests/fixtures/handwritten_math/manifest.json), [evaluation command](../../scripts/evaluate_handwritten_math_corpus.py) | [corpus, policy, and replay tests](../../tests/test_handwritten_math_suitability.py) |
