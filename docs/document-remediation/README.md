...
General STEM remediation is a PDF specialist layer rather than another input format. It binds each accepted visual to exact source evidence, applies only the matching typed specialist, reopens the saved PDF for reverse verification, and leaves the result unavailable until human approval of the exact current candidate. See the [General STEM guide](general-stem.md) for supported visual classes and refusal boundaries.
...
| Mixed STEM composition | [region graph](../../src/education/mixed_stem_regions.py), [router](../../src/education/remediation/mixed_stem_region_router.py), [composition contract](../../src/education/mixed_stem_composition.py), [composer](../../src/education/remediation/mixed_stem_composer.py) | [routing](../../tests/test_mixed_stem_region_routing.py), [composition](../../tests/test_mixed_stem_composition.py) |
