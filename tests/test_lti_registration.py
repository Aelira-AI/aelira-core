...
def mock_department_free():
    """Create a mock department with free tier (no lms_integration)."""
    dept = MagicMock(spec=Department)
    dept.id = "dept-free-456"
    dept.name = "Free Department"
    dept.tier = "individual_free"
    dept.is_active = True
    return dept
    ...
    def test_free_tier_denied(self, mock_check_feature, mock_department_free):
        ...
        with patch(
            "src.config.settings.TIER_QUOTAS",
            {
                ...
                "individual_free": {"features": [], "excluded": ["lms_integration"]},
            },
        ):
        ...
        assert "individual_free" in error
...
class TestRenderLTIErrorPage:
    ...
    def test_error_page_with_upgrade_button(self):
        ...
        assert "View Plans & Pricing" in html
...
class TestLTILaunchFlow:
    ...
    def test_complete_launch_feature_denied(
        ...
        assert error is None
        assert department is not None
        assert department.tier == "individual_free"
