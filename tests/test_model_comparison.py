...
def call_ollama(
    model: str, prompt: str, system: str = None, max_tokens: int = 500
) -> dict:
...
def run_speed_benchmark():
    ...
    prompt = (
        "Explain in one sentence what WCAG 2.1 AA compliance means for universities."
    )
        ...
        if result["success"]:
            results.append(
                (
                    ...
                    result["content"].strip()[:100],
                )
            )
            print(
                f"  {model:25s} — {result['elapsed']:5.1f}s — {result['tokens_per_sec']:5.0f} tok/s"
            )
        else:
