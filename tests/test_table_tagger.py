...
class TestHeaderIdentification:
    ...
    def test_first_row_all_non_empty_is_header(self):
        ...
        table = TableInfo(page_num=0, bbox=(0, 0, 500, 300), rows=3, cols=3)
        table.cells = [
    ...
    def test_first_row_with_empty_cell_not_header(self):
        ...
        tagger = TableTagger(use_ai=False)
        table = TableInfo(page_num=0, bbox=(0, 0, 500, 300), rows=2, cols=3)
        table.cells = [
            CellInfo(0, 0, "Name", (0, 0, 100, 20)),
        ...
        col_scope_row0 = [
            c for c in table.cells if c.is_header and c.row == 0 and c.scope == "Column"
        ]
    ...
    def test_empty_table_no_crash(self):
        tagger = TableTagger(use_ai=False)
        table = TableInfo(page_num=0, bbox=(0, 0, 500, 300), rows=0, cols=0)
        # Should not raise
        tagger._identify_headers_heuristic(table)
        assert table.header_rows == 0
    ...
    def test_long_text_not_header(self):
        ...
        tagger = TableTagger(use_ai=False)
        table = TableInfo(page_num=0, bbox=(0, 0, 500, 300), rows=2, cols=2)
...
class TestComplexityAssessment:
    ...
    def test_no_headers_lower_score(self):
        tagger = TableTagger(use_ai=False)
        table = TableInfo(page_num=0, bbox=(0, 0, 500, 200), rows=3, cols=3)
...
class TestBboxOverlap:
    ...
    def test_no_overlap(self):
        assert TableTagger._bbox_overlap((0, 0, 50, 50), (100, 100, 200, 200)) == 0.0
    ...
    def test_contained_box(self):
        overlap = TableTagger._bbox_overlap((0, 0, 200, 200), (50, 50, 100, 100))
        assert 0.0 < overlap < 1.0

    def test_zero_area_box(self):
        assert TableTagger._bbox_overlap((0, 0, 0, 0), (0, 0, 100, 100)) == 0.0
        ...
        ai_content = json.dumps(
            {
                "header_rows": [0],
                "header_cols": [],
                "merged_cells": [{"row": 0, "col": 0, "col_span": 2, "row_span": 1}],
            }
        )
        tagger._apply_ai_headers(table, ai_content)

        cell_00 = next(c for c in table.cells if c.row == 0 and c.col == 0)
