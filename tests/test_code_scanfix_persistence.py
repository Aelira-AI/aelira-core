...
def _map_code_issue_to_scanfix_fields(issue_dict: dict, scan_id: str = "test-scan") -> dict:
...
class TestFixContentFallback:
    ...
    def test_ai_fix_preferred(self, full_code_issue):
        fields = _map_code_issue_to_scanfix_fields(full_code_issue)
        assert fields["fixed_content"] == '<img src="banner.png" alt="Campus welcome banner">'
    ...
    def test_heuristic_fallback_when_no_ai_fix(self, heuristic_code_issue):
        ...
        assert fields["fixed_content"] == "Add a <label> element with a for attribute matching the input id"
...
class TestCategoryMapperDelegation:
    ...
    def test_html_image_alt_maps_to_alt_text(self):
        issue = {"category": "html", "rule": "image-alt", "severity": "critical", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "alt_text"

    def test_html_heading_hierarchy_maps_to_heading(self):
        issue = {"category": "html", "rule": "heading-hierarchy", "severity": "moderate", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "heading"

    def test_html_form_label_maps_to_form(self):
        issue = {"category": "html", "rule": "form-label", "severity": "moderate", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "form"

    def test_html_lang_attribute_maps_to_language(self):
        issue = {"category": "html", "rule": "lang-attribute", "severity": "minor", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "language"

    def test_html_page_title_maps_to_title(self):
        issue = {"category": "html", "rule": "page-title", "severity": "moderate", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "title"

    def test_css_focus_indicator_maps_to_navigation(self):
        issue = {"category": "css", "rule": "focus-indicator", "severity": "moderate", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "navigation"

    def test_css_color_contrast_maps_to_contrast(self):
        issue = {"category": "css", "rule": "color-contrast", "severity": "serious", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "contrast"

    def test_aria_wildcard_maps_to_aria(self):
        issue = {"category": "aria", "rule": "any-rule-name", "severity": "moderate", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
        assert fields["category"] == "aria"

    def test_unknown_category_rule_defaults_to_structure(self):
        issue = {"category": "unknown", "rule": "unknown-rule", "severity": "moderate", "description": "x"}
        fields = _map_code_issue_to_scanfix_fields(issue)
...
class TestIssueIdGeneration:
    ...
    def test_same_issue_produces_same_id(self):
        issue = {"category": "html", "rule": "image-alt", "severity": "serious", "description": "Missing alt text"}
    ...
    def test_different_descriptions_produce_different_ids(self):
        issue_a = {"category": "html", "rule": "image-alt", "severity": "serious", "description": "Missing alt text"}
        issue_b = {"category": "html", "rule": "image-alt", "severity": "serious", "description": "Empty alt text"}
    ...
    def test_different_rules_produce_different_ids(self):
        issue_a = {"category": "html", "rule": "image-alt", "severity": "serious", "description": "x"}
        issue_b = {"category": "html", "rule": "form-label", "severity": "serious", "description": "x"}
        id_a = _map_code_issue_to_scanfix_fields(issue_a)["issue_id"]
        id_b = _map_code_issue_to_scanfix_fields(issue_b)["issue_id"]
        assert id_a != id_b
