...
class TestEmailPreferencesValidation:
    ...
    def test_partial_update_preserves_other_fields(self, client):
        ...
        if get_response.status_code == 200:
            ...
            client.patch(
                "/auth/profile/email-preferences",
                json={"email_marketing": not original.get("email_marketing", False)},
            )
...
class TestEmailFilteringByPreference:
    ...
    @pytest.mark.skip(reason="mock filter cannot match SQLAlchemy condition objects to email strings")
