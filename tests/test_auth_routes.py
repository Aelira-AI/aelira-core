...
class TestSignupIndividual:
    """POST /auth/signup/individual (L647-891) — public."""

    def test_non_edu_email_is_422(self, client, mock_db):
        # L100-118: validate_edu_email rejects non-.edu domains.
        response = client.post(
            "/auth/signup/individual",
            json={"email": "prof@gmail.com", "name": "Prof X"},
        )
        assert response.status_code == 422

    def test_existing_email_is_400(self, client, monkeypatch):
        # L730-736: an existing User with the same email -> 400.
        # (signup_individual has no separate RateLimiter check either.)
        monkeypatch.setattr(
            auth_routes,
            "check_signup_abuse",
            AsyncMock(
        ...
        _use_db(_db_with({User: {"first": existing_user}}))
        response = client.post(
            "/auth/signup/individual",
            json={"email": EDU_EMAIL, "name": "Prof X"},
        )
        assert response.status_code == 400

    def test_free_account_limit_reached_is_403(self, client, monkeypatch):
        # L738-759: Department count >= FREE_ACCOUNT_LIMIT -> 403.
        from src.config.settings import FREE_ACCOUNT_LIMIT
        ...
        _use_db(
            _db_with(
                {
                    User: {"first": None},
                    Department: {"count": FREE_ACCOUNT_LIMIT},
                }
            )
        )
        response = client.post(
        ...
        assert response.status_code == 403
        assert response.json()["detail"]["error"] == "free_accounts_full"

    def test_abuse_challenge_required_is_403(self, client, monkeypatch):
        # L715-728: recommended_action == "challenge" -> 403 with challenge details.
        monkeypatch.setattr(
            auth_routes,
            "check_signup_abuse",
            AsyncMock(
        ...
        assert response.json()["detail"]["challenge_type"] == "captcha"

    def test_happy_path_issues_api_key(self, client, monkeypatch):
        # L774-826: on success, creates department+user+API key (both
        # constructed with an explicit id=str(uuid.uuid4()), unlike
        # create_department's Department -- see _db_with's refresh backfill).
        # L881-891: returns the full key once (mirrors CreateAPIKeyResponse.full_key at L312).
        monkeypatch.setattr(
            auth_routes,
            "check_signup_abuse",
            AsyncMock(
        ...
        monkeypatch.setattr(
            auth_routes,
            "get_quota_status",
            lambda db, dept_id: {"tier": "individual_free"},
        )
        email_service = MagicMock()
        ...
        assert response.status_code == 200
        body = response.json()
        # L881-891: response echoes success, the one-time key, and its prefix.
        assert body["success"] is True
        assert body["api_key"].startswith("aelira_live_")
        assert body["key_prefix"] == body["api_key"][:12]
        assert body["tier"] == "individual_free"


# ==================== Magic link (L977-1264) ====================
...
class TestMagicLinkRequest:
    ...
    def test_non_edu_email_is_422(self, client, mock_db):
        # L946-950: validate_edu_email rejects non-.edu domains.
        response = client.post(
            "/auth/magic-link/request", json={"email": "someone@gmail.com"}
        )
        assert response.status_code == 422
